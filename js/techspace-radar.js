function loadData() {
    ...
    d3.csv("data/radar.csv", function(t) {
        t = _.each(t, function(e) {
            e.Orbit = e.Orbit.split(", ").join(" "), e["Business Model"] = e["Business Model"].split("|"), e["Channel"] = e["Business Model"], e.Industry = e.Industry.replace("| ", "|").trim().split("|"), keywords = keywords.concat(e.Industry), tags = tags.concat(e.Industry), keywords.push(e["Company Name"].trim()), e.Industry = e.Industry.join(" ")
...
function showCompanyInfo(e, t, n) {
    ...
    if(e["Channel"].length >= 1){
        r.find(".channel").html("<strong>Distrobution Channel :</strong> "+e["Channel"].join(", ")); 
    }
    r.find(".orbit").html("");
